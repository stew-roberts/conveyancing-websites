...
const Hero = ({details}) => {
  ...
  return (
    <div className="flex items-center justify-center h-screen bg-fixed bg-center bg-cover" style={{ backgroundImage: `url(${backgroundImage})`}}>
        ...
        <div className="max-w-lg md:ml-[-50%] backdrop-blur-md">
          <h2 className="text-white text-4xl z-[3] flex flex-wrap leading-normal drop-shadow-lg">{details.title}</h2>
          <p className="text-white mt-8 drop-shadow-lg">{details.description}</p>
        </div>
    </div>
  )
}
